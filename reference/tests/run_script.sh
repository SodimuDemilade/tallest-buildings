#!/usr/bin/env bash
# Behavioral verifier runner (OFFLINE) for task_0002-tallest-buildings-rank-silhouette.
#
...
TEST_SPEC="${TEST_SPEC:-buildings-rank-silhouette.spec.js}"
...
APP_DIR="${APP_DIR:-}"
if [ -z "$APP_DIR" ]; then
  for d in "/app/tallest-buildings-rank-silhouette" "/app" "$(pwd)/tallest-buildings-rank-silhouette" "$(pwd)"; do
